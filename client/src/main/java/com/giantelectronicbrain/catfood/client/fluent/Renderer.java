...
//					((Element) newView.element).getStyle().setProperty(name.nameValid(), newView.styles.get(name));
					FluentBase.setStyle((Element)newView.element, name, newView.styles.get(name));
				}
			}
			if (newView.text != null) {
				newView.element.textContent = newView.text;
			}
			...
			if(newView.innerHTML != null) {
				Fluent.console.log("This fluent has innerHTML, adding it to the element");
				Element e = (Element) newView.element;
				e.innerHTML = newView.innerHTML;
			}
		}
	...
	private static void renderChanges(Fluent parent, Fluent newView, Fluent oldView) {
		...
		if (parent.element != null) { // if we are dom-attached
			...
			if(!equalsString(newView.innerHTML,oldView.innerHTML)) {
				Fluent.console.log("replacing a view with a new view that has innerHTML, setting it to ",oldView.innerHTML);
				((Element)newView.element).innerHTML = newView.innerHTML;
				return; // you wouldn't have children if you have innerHTML...
			}
		}
	...
	private static <T, V> void compareApplyRemove(Node element, T name, V value, What what) {
		...
		case Attributes:
			...
			case checked:
				...
				break;
			case value:
				((HTMLInputElement) element).value = null;
				break;
			case selectedIndex:
				// nothing to do
...
//			((Element) element).getStyle().removeProperty(((Css) name).nameValid());
			FluentBase.removeStyle((Element)element, (Css)name);
			break;
		case Listeners:
	...
	private static <T, V> void compareApplySet(Node element, T name, V value, What what) {
		...
		case Attributes:
			...
			case value:
				...
				break;
			case selectedIndex:
				((HTMLSelectElement) element).selectedIndex = Integer.parseInt((String) value);
			default:
...
//			((Element) element).getStyle().setProperty(((Css) name).nameValid(), (String) value);
			FluentBase.setStyle((Element)element, (Css) name, (String) value);
			break;
		case Listeners:
